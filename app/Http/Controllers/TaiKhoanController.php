...
class TaiKhoanController extends Controller
{
    public function viewQuanLy(Request $request)
    {
        ...
        // Tạo query cơ bản với join bảng ql_khachhang
        ...
        if ($request->has('search_name') && !empty($request->search_name)) {
            $query = $query->where('ho_ten', 'like', '%' . $request->search_name . '%')
                ->orWhere('ten_nhan_vien', 'like', '%' . $request->search_name . '%');
            $data['search_name'] = $request->search_name;
        }

        // Tìm kiếm khách hàng hay nhân viên
        if ($request->has('search_customer') && !empty($request->search_customer)) {
            if ($request->search_customer == 1) {
                $query = $query->whereNull('id_khach_hang');
            } else {
                $query = $query->whereNotNull('id_khach_hang');
            }
        ...
        if ($request->has('search_status') && !empty($request->search_status)) {
            if ($request->search_status == 'active') {
                $query = $query->where('trang_thai', 1);
            } else {
                $query = $query->where('trang_thai', 0);
            }
