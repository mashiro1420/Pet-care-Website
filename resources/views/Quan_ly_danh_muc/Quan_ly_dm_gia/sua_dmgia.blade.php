...
<body>
  ...
  <div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h3 class="mb-0">Cập nhật giá</h3>
      <a href="{{ route('ql_dmgia') }}" class="btn btn-outline-secondary">
      ...
      <div class="card-body p-4">
        <form action="{{ url('xl_sua_dmdichvu') }}" method="POST">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              <div class="mb-3">
                <label for="role" class="form-label">Tên dịch vụ</label>
                <select class="form-select" name="ten_dich_vu" required>
                  @foreach ($dich_vus as $dich_vu)
                  <option value="{{ $dich_vu->id }}" {{ $gia->id_dich_vu==$dich_vu->id?"selected":"" }}>{{ $dich_vu->ten_dich_vu }}</option>
                  @endforeach
                </select>
              </div>
            </div>
