...
<head>
  ...
  <title>Chi tiết bài đăng - PetCare</title>
...
<body>
  ...
  <div class="post-meta mb-3">
    ...
    <p><strong>Trạng thái:</strong> {{ $bai_dang->trang_thai == 1 ? 'Hiển thị' : 'Ẩn' }}</p>
  ...
  @if($bai_dang->hinh_anh)
    <img src="{{ asset($bai_dang->hinh_anh) }}" alt="Hình ảnh chi tiết" class="post-image mb-3">
  @endif
  ...
  @if($bai_dang->link_video)
    <div class="mb-4">
      ...
      <div class="ratio ratio-16x9">
        <iframe src="{{ $bai_dang->link_video }}" frameborder="0" allowfullscreen></iframe>
      </div>
    </div>
  @endif
  ...
  <div class="d-flex align-items-center gap-3 mt-3">
    <button id="likeButton" class="btn btn-outline-danger btn-sm">
      <i class="bi bi-hand-thumbs-up"></i> Thích (<span id="likeCount">{{ $bai_dang->luot_thich }}</span>)
    </button>
  </div>


    <a href="{{ route('baidang.user') }}" class="btn btn-back mt-4">
